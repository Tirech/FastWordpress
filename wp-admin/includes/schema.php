...
CREATE TABLE $wpdb->term_relationships (
 ...
 CONSTRAINT term_order UNIQUE (term_order)
);
...
CREATE TABLE $wpdb->comments (
  ...
  comment_date_gmt timestamp without time zone NOT NULL default 'now()',
...
CREATE TABLE $wpdb->links (
  ...
  link_updated timestamp without time zone NOT NULL default 'now()',
...
CREATE TABLE $wpdb->posts (
  ...
  post_date_gmt timestamp without time zone NOT NULL default 'now()',
  ...
  post_modified timestamp without time zone NOT NULL default 'now()',
  post_modified_gmt timestamp without time zone NOT NULL default 'now()',
...
CREATE TABLE $wpdb->signups (
  ...
  user_email varchar(100) NOT NULL default '',
  registered timestamp without time zone NOT NULL default 'now()',
  activated timestamp without time zone NOT NULL default 'now()',
...
file_put_contents('create_table.sql',$wp_queries);
...
//	        AND b.option_value < $time");
    $wpdb->query("DELETE  FROM $wpdb->options WHERE option_id IN(
	          ...
	          AND (CASE WHEN b.option_value ~ '^[0-9]+$' THEN b.option_value::int8 ELSE 0 END) < $time
	          UNION ALL
              SELECT b.option_id
              FROM $wpdb->options a, $wpdb->options b
              WHERE a.option_name ILIKE '_transient_%'
        ...
        $wpdb->query("DELETE FROM $wpdb->options
			...
			FROM $wpdb->options a, $wpdb->options b
			 WHERE a.option_name ILIKE '_site_transient_%'
			  ...
			  AND (CASE WHEN b.option_value ~ '^[0-9]+$' THEN b.option_value::int8 ELSE null END) < $time
			  UNION  ALL
			SELECT b.option_id
			FROM $wpdb->options a, $wpdb->options b
			 WHERE a.option_name ILIKE '_site_transient_%'
